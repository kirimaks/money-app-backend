import { Logger, UseGuards, InternalServerErrorException, NotFoundException } from '@nestjs/common';
...
import { INTERNAL_SERVER_ERROR, CATEGORY_NOT_FOUND_ERROR } from '../errors/constants';
import { CategoryNotFoundError } from '../errors/category';

import type { CategoryRepresentation } from './category.types';
import type { GetCategoryInput, CreateCategoryInput } from './category.validation';
...
export class CategoryResolver {
  ...
  async category(
    ...
    try {
      return await this.categoryService.getCategory(user.id, getCategoryInput.id);
    } catch(error) {
  ...
  async createCategory(
    @Args(new ZodPipe(createCategorySchema)) createCategoryInput: CreateCategoryInput,
    @CurrentUser() user: UserInRequest
  ):Promise<CategoryRepresentation> {
    try {
