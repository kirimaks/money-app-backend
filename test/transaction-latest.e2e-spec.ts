...
describe('Latest transactions test', () => {
  ...
  test('Latest transactions 1 day test', async () => {
    ...
    const { data } = await request<{
      latestTransactions: LatestTransactionsByDay[];
    }>(app.getHttpServer())
  ...
  test('Latest transactions pagination', async () => {
    ...
    const secondPageResp = await request<{
      latestTransactions: LatestTransactionsByDay[];
    }>(app.getHttpServer())
    ...
    expect(
      secondPageResp.data?.latestTransactions[0]?.transactions,
    ).toHaveLength(2);
    expect(secondPageResp.data?.latestTransactions[0]?.totalAmount).toEqual(
      1100,
    );

    expect(
      secondPageResp.data?.latestTransactions[1]?.transactions,
    ).toHaveLength(3);
    expect(secondPageResp.data?.latestTransactions[1]?.totalAmount).toEqual(
      600,
    );
  });
});
